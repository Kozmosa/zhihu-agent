...
def test_capture_preview_then_explicit_import_preserves_markdown_and_author_scope(companion):
    ...
    assert (
        len(client.get("/api/v1/sources", params={"author_id": "zhihu-author:people:alice"}).json())
        == 2
    )
    assert (
        len(client.get("/api/v1/sources", params={"author_id": "zhihu-author:people:bob"}).json())
        == 1
    )
