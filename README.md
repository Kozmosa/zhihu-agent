...
已配置凭据的检测和脱敏覆盖明文以及有限层数的 URL、HTML 实体和 JSON Unicode 转义。只替换匹配密钥的原始片段，保留其他用户文本；模型回传凭据会被拒绝，知乎搜索摘要在 HTML 清洗后也会再次检查。模型名称、地址等会回显的配置字段不得包含已配置密钥的这些编码形式。

提交前可在当前仓库启用本地检查（不修改全局 Git 设置）：
...
检查读取 Git 暂存区里的内容，因此暂存后再从工作文件删除密钥也不能绕过检查。它拦截本机模型配置、环境文件、私钥、数据库、Cookie 导出，以及可识别的硬编码凭据；结果只列文件、行号和规则，不打印命中内容。测试数据按具体文件和具体合成值豁免，不放行整个测试目录。

GitHub 的 `Credential guard` 工作流在 push 和 PR 上检查提交快照，使用只读权限且不向扫描服务上传文件。若要强制禁止合并失败的检查，需将该检查设为仓库分支规则中的必需检查。此扫描不保证发现任意格式、任意编码或历史提交中的所有秘密；已经公开的密钥仍需在服务商处撤销并更换，删除代码不能使旧密钥失效。

后续功能分支和 PR 使用 `guoyin/` 命名；这不会修改既有提交作者或历史记录。
