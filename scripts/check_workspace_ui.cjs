...
(async () => {
  ...
  assert.equal(get('library-question-title-form').hidden, false);
  get('library-question-title').value = '补全后的实际问题？';
  failures.set('/api/v1/sources/question-title', '保存失败');
  await submit('library-question-title-form');
  assert.match(get('library-scope').textContent, /管理测试问题/);
  assert.equal(get('library-question-title').value, '补全后的实际问题？');
  failures.delete('/api/v1/sources/question-title');
  await submit('library-question-title-form');
  assert.match(get('library-scope').textContent, /补全后的实际问题？ · 2 篇回答/);
  await click('library-back');
  assert.match(allText(get('source-list')), /补全后的实际问题？/);
  get('library-search').value = '88001'; await submit('library-search-form');
  await get('source-list').children[0].click(); await idle();
