...
1. 管理员启动服务后进入 `/admin` 服务管理页，找到“知乎连接”区域。
2. 登录[知乎开放平台](https://developer.zhihu.com/)，在个人中心获取 Access Secret；在管理页填写并保存。随后用户从 `/workspace` 点击“搜索知乎”使用。精简桌面浮窗可先点击“完整工作台”进入，用户页面不展示凭证配置。
...
另有“导入知乎问题”入口，可在专用浏览器窗口中从一个问题读取多篇已加载回答，预览后
批量保存。它使用网页登录状态，不使用此处的搜索 API 或 Access Secret；每批最多 20 篇，
不承诺覆盖整个问题。读取到的正文标为 `content_extent=unknown`，不会推断为完整正文。
首次登录与平台验证由用户完成；专用 WebView2 配置位于资料目录的 `zhihu-browser` 中，
应用不会读取或输出 Cookie。读取任务暂存数据在 `zhihu-question-jobs`，不会自动进入资料库。
