...
def to_preview(capture: BrowserCapture) -> ZhihuWebPreviewResult:
    ...
    for record in capture.items:
        ...
        # Match the desktop reader's profile identity, including people/org.
        author_kind = zhihu_url(record.author_url).path.split("/")[1] if slug else None
        external_author_id = f"{author_kind}:{slug}" if slug else None
        page_author_kind = page.path.split("/")[1] if profile else None
        ...
        if capture.scope == "author" and (
            record.content_type != "answer"
            or slug != capture.scope_id
            or author_kind != page_author_kind
        ):
        ...
        if capture.scope == "page":
            ...
            same_author = (
                ...
                and record.content_type == "answer"
                and slug == profile[1]
                and author_kind == page_author_kind
                and page.hostname != "zhuanlan.zhihu.com"
            )
        ...
        try:
            draft = SourceDraft(
                ...
                author_id=f"zhihu-author:{external_author_id}"
                if slug
