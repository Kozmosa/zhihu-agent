...
def test_provider_reflected_credentials_are_redacted_from_both_transcript_events(provider, key):
    events = []
    ...
    try:
        with httpx.Client(
            ...
            result = transport(provider, client, model="fixture-" + key).request(
    ...
    assert json.loads(events[1]["payload"]["response"])["debug"]["authorization"] == "[redacted]"
